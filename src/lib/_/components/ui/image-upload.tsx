...
import { deleteImageAction } from "../../cloudinary/cloudinary.actions";
...
export function ImageUpload(props: ImageUploadProps) {
  ...
  const responsiveHeightClass = "h-[min(30vh,300px)]";
  ...
  if (!props.multiple) {
    ...
    return (
      <div className="w-full space-y-2">
        <label className="text-muted-foreground">Télécharger une image</label>
        {value ? (
          <div className="relative w-full">
            <div
              className={`relative w-full overflow-hidden rounded-xl border border-gray-200 shadow-sm ${responsiveHeightClass}`}
            >
              {publicId ? (
                <CldImage
                  src={publicId}
          ...
          >
            {({ open }) => (
              <button
                type="button"
                onClick={() => open()}
                className={`mt-1.5 flex w-full flex-col items-center justify-center rounded-xl border-2 border-dashed transition ${s.zone} ${responsiveHeightClass}`}
              >
                <Upload size={24} className={s.icon} />
                <span className={`mt-2 text-sm font-semibold ${s.text}`}>
                  Choisir une image
                </span>
                <span className={`text-[10px] font-semibold ${s.subtext}`}>
                  JPG, PNG, WebP
                </span>
              </button>
            )}
        ...
        <div className="flex flex-wrap gap-3">
          {values.map((img) => (
            <div key={img.publicId} className="relative">
              <div className="relative h-32 w-32 overflow-hidden rounded-xl border border-gray-200 shadow-sm">
                <CldImage
                  src={img.publicId}
                  alt="Image"
                  fill
      ...
      >
        {({ open }) => (
          <button
            type="button"
            onClick={() => open()}
            className={`flex h-32 w-32 flex-col items-center justify-center rounded-xl border-2 border-dashed transition ${s.zone}`}
          >
