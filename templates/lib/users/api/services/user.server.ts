...
export async function getUsers( filters: UserFilters,): Promise<Result<UsersResponse, ApiError>> {
  try {
...
export async function createUser(
  ...
  if (!session?.user || session.user.role !== "ADMIN") {
    logger.warn(
      { context: "createUser" },
      "Unauthorized: only ADMIN users can create new users",
    );
    return {
      ok: false,
...
export async function updateUser(
  ...
  if (!session?.user || session.user.role !== "ADMIN") {
    logger.warn(
      { context: "updateUser" },
      "Unauthorized: only ADMIN users can update users",
    );
    return {
      ok: false,
   ...
   */
  try {
    const res = await apiClient(false, config).patch<User>(
      `${usersUrl}/${id}`,
      parse.data,
    );
