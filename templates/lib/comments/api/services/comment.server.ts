...
export async function getComments(
  ...
  try {
    ...
    const url = `${commentsUrl}${queryParams ? `?${queryParams}` : ""}`;

    const res = await apiClient(true).get<CommentsResponse>(url);
    logger.info({ count: res.data.meta.total }, "get comments");
...
export async function getCommentById(
  ...
  try {
    ...
    logger.error({ id }, "Failed to get comment");
    return {
      ok: false,
...
export async function deleteComment(
   ...
   */
  try {
    await apiClient(false, config).delete(`${commentsUrl}/${id}`);
